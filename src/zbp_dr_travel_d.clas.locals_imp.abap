CLASS lhc_ZDR_R_TRAVEL_D DEFINITION INHERITING FROM cl_abap_behavior_handler.
  PRIVATE SECTION.

    METHODS get_instance_authorizations FOR INSTANCE AUTHORIZATION
      IMPORTING keys REQUEST requested_authorizations FOR zdr_r_travel_d RESULT result.

    METHODS get_global_authorizations FOR GLOBAL AUTHORIZATION
      IMPORTING REQUEST requested_authorizations FOR zdr_r_travel_d RESULT result.

    METHODS precheck_create FOR PRECHECK
      IMPORTING entities FOR CREATE zdr_r_travel_d.

    METHODS precheck_update FOR PRECHECK
      IMPORTING entities FOR UPDATE zdr_r_travel_d.

    METHODS accepttravel FOR MODIFY
      IMPORTING keys FOR ACTION zdr_r_travel_d~accepttravel RESULT result.

    METHODS deductdiscount FOR MODIFY
      IMPORTING keys FOR ACTION zdr_r_travel_d~deductdiscount RESULT result.

    METHODS recaltotalprice FOR MODIFY
      IMPORTING keys FOR ACTION zdr_r_travel_d~reCalcTotalPrice.

    METHODS rejecttravel FOR MODIFY
      IMPORTING keys FOR ACTION zdr_r_travel_d~rejecttravel RESULT result.

    METHODS calctotprice FOR DETERMINE ON MODIFY
      IMPORTING keys FOR zdr_r_travel_d~calctotprice.

    METHODS setstatusopen FOR DETERMINE ON MODIFY
      IMPORTING keys FOR zdr_r_travel_d~setstatusopen.

    METHODS settravelid FOR DETERMINE ON SAVE
      IMPORTING keys FOR zdr_r_travel_d~settravelid.

    METHODS validatecustomer FOR VALIDATE ON SAVE
      IMPORTING keys FOR zdr_r_travel_d~validatecustomer.

    METHODS activate FOR MODIFY
      IMPORTING keys FOR ACTION zdr_r_travel_d~activate.

    METHODS discard FOR MODIFY
      IMPORTING keys FOR ACTION zdr_r_travel_d~discard.

    METHODS edit FOR MODIFY
      IMPORTING keys FOR ACTION zdr_r_travel_d~edit.

    METHODS resume FOR MODIFY
      IMPORTING keys FOR ACTION zdr_r_travel_d~resume.

ENDCLASS.

CLASS lhc_ZDR_R_TRAVEL_D IMPLEMENTATION.
...
*    ENDIF.
  ...
  METHOD deductDiscount.
    ...
    DATA : lv_disc       TYPE decfloat16,
           lt_travel_new TYPE TABLE FOR UPDATE zdr_r_travel_d.

    DATA(lt_keys) = keys.

    LOOP AT lt_keys ASSIGNING FIELD-SYMBOL(<lt_keys>)
     WHERE %param-discount IS INITIAL
     OR %param-discount GT 100
     OR %param-discount LE 0.
      APPEND VALUE #( %tky = <lt_keys>-%tky ) TO failed-zdr_r_travel_d.
      APPEND VALUE #( %tky = <lt_keys>-%tky
      %msg = NEW /dmo/cm_flight_messages(
    ...
    LOOP AT lt_travel ASSIGNING FIELD-SYMBOL(<ls_travel>).
      ...
      APPEND VALUE #( %tky = <ls_travel>-%tky BookingFee = lv_dic_book_fee ) TO lt_travel_new.
    ENDLOOP.
    MODIFY ENTITIES OF zdr_r_travel_d IN LOCAL MODE
    ENTITY zdr_r_travel_d
    ...
    result = VALUE #( FOR ls_no_travel IN lt_modified_travel
    ( %tky = ls_no_travel-%tky
      %param = ls_no_travel ) ).
  ENDMETHOD.
  ...
  METHOD recalTotalPrice.
    ...
    DATA : lt_amt_per_code TYPE STANDARD TABLE OF ty_amount_per_currencycode.
    READ ENTITIES OF zdr_r_travel_d IN LOCAL MODE
    ENTITY zdr_r_travel_d
    ...
    LOOP AT lt_travels ASSIGNING FIELD-SYMBOL(<travel>).
      lt_amt_per_code = VALUE #( ( amount = <travel>-BookingFee
      ...
      LOOP AT lt_bookings INTO DATA(booking) WHERE CurrencyCode IS NOT INITIAL.
        COLLECT VALUE ty_amount_per_currencycode( amount = booking-FlightPrice
                                                  ...
                                                  ) INTO lt_amt_per_code.
      ENDLOOP.
      CLEAR <travel>-TotalPrice.
      LOOP AT lt_amt_per_code INTO DATA(Single_amount_per_currencycode).
